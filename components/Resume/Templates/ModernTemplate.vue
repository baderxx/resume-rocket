...
type PartialResume = Partial<ResumeInformation> & {
  ...
  socialLinks?: ResumeInformation["socialLinks"];
};
...
<template>
  <div class="flex h-full overflow-hidden rounded bg-white text-gray-900">
    <aside class="flex w-64 flex-col justify-between bg-gradient-to-b from-indigo-900 to-slate-900 p-6 text-white">
      <div class="space-y-3">
        <div>
          <p class="text-xl font-bold leading-tight">{{ fullName }}</p>
          <p class="text-sm text-indigo-100">{{ resume.jobTitle || "Role / Title" }}</p>
        </div>
        ...
        <div v-if="safeSocialLinks.length" class="pt-2">
          <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-indigo-200">Links</p>
          <ul class="mt-2 space-y-1 text-sm text-indigo-100/90">
            <li v-for="(link, index) in safeSocialLinks" :key="index">
              {{ link.label }}
              <span v-if="link.url" class="block text-indigo-200/80">{{ link.url }}</span>
            </li>
          </ul>
        </div>
      </div>
      <div class="space-y-2">
        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-indigo-200">Skills</p>
        ...
        <p v-else class="text-xs text-indigo-100/80">Add skills to show your strengths.</p>
      </div>
    </aside>

    <main class="flex-1 space-y-6 p-7">
      <section class="rounded-lg border border-indigo-100 bg-indigo-50/60 p-4">
        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-800">
          Profile
        </p>
      ...
      <section>
        <div class="flex items-center justify-between">
          <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Experience</p>
          <div class="h-px flex-1 bg-gradient-to-r from-slate-300/70 to-transparent" />
        </div>
        <div v-if="safeHistory.length" class="mt-3 space-y-4">
          ...
          >
            <div class="flex flex-wrap items-baseline justify-between gap-2">
              <p class="text-sm font-semibold text-slate-900">{{ item.jobTitle || "Job title" }}</p>
            ...
            <p class="mt-1 text-sm leading-relaxed text-slate-700">
              {{ item.description || "Describe the results and responsibilities for this role." }}
            </p>
          </article>
        </div>
        <p v-else class="mt-2 text-sm text-slate-500">Add your experience to show your impact.</p>
      </section>
      ...
      <section>
        ...
        <div v-if="safeProjects.length" class="mt-3 grid gap-3 md:grid-cols-2">
          ...
          >
            ...
            <p v-if="project.url" class="text-xs text-indigo-700">{{ project.url }}</p>
            <p class="mt-1 text-sm leading-relaxed text-slate-700">{{ project.description }}</p>
          </article>
        </div>
        <p v-else class="mt-2 text-sm text-slate-500">Add projects to highlight extra wins.</p>
      </section>
      ...
      <section>
        ...
        <p v-else class="mt-2 text-sm text-slate-500">Add your education background.</p>
      </section>
    </main>
  </div>
</template>
